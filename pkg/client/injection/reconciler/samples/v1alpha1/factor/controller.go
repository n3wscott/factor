...
import (
	...
	factor "knative.dev/sample-controller/pkg/client/injection/informers/samples/v1alpha1/factor"
)

const (
	defaultControllerAgentName = "factor-controller"
	defaultFinalizerName       = "factors.samples.knative.dev"
)
...
func NewImpl(ctx context.Context, r Interface, optionsFns ...controller.OptionsFn) *controller.Impl {
	...
	factorInformer := factor.Get(ctx)

	lister := factorInformer.Lister()
	...
	logger = logger.With(
		...
		zap.String(logkey.Kind, "samples.knative.dev.Factor"),
	)
